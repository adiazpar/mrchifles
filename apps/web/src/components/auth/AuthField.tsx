...
interface AuthFieldProps extends Omit<InputHTMLAttributes<HTMLInputElement>, 'children'> {
  ...
  // used for an inline hint or per-field error.
  below?: ReactNode
}

export const AuthField = forwardRef<HTMLInputElement, AuthFieldProps>(
  function AuthField(
    { label, id, className, trailing, below, type, ...inputProps },
    ref,
  ) {
    const inputId =
    ...
    return (
      <label className="auth-field" htmlFor={inputId}>
        ...
        <input {...inputProps} type={type} id={inputId} ref={ref} className={inputClass} />
